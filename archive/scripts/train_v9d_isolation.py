#!/usr/bin/env python3
"""
V9D Variable Isolation Test Training
====================================

SCIENTIFIC ISOLATION: Test if v9C's 92x improvement came from CAPACITY or BATCH SIZE.

v9C changed TWO variables simultaneously:
- LoRA r: 16 → 32
- batch_size: 4 → 1

v9D isolates by using:
- LoRA r: 32 (like v9C - HIGH capacity)
- batch_size: 4 (like v9B - NORMAL batching)

EXPECTED OUTCOMES:
- If v9D ≈ v9C (high AGL): Capacity (r=32) is the key! → Use r=32 overnight
- If v9D ≈ v9B (low AGL): Batch regularization is the key! → Use batch=1 overnight
- If v9D is between: BOTH factors matter → Use r=32 AND batch=1 overnight
...
class TrainingConfig:
    """V9D Variable Isolation Test configuration - ISOLATION TEST - r=32 with batch=4!"""
    # Model
    ...
    batch_size: int = 4  # Back to 4 (like v9B) - testing if capacity OR batch caused v9C success
    gradient_accumulation_steps: int = 4  # Back to 4 (like v9B) - effective batch 16 maintained
    num_epochs: int = 3
    ...
    lora_r: int = 32       # Was 16 in v9B
    lora_alpha: int = 64   # Was 32 in v9B
    lora_dropout: float = 0.05
    
    # Output
    output_dir: str = "exports/v9d_isolation"
...
def train_v9d_isolation(config: TrainingConfig):
    """Run V9D Variable Isolation Test training."""
    print("=" * 60)
    print("🧠 V9D CAPACITY TEST - r=32 + batch=4 (isolating variables)!")
    print("=" * 60)
    ...
    training_args = TrainingArguments(
        ...
        report_to="none",
        run_name=f"v9d_isolation_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    # Train!
    print("\n" + "=" * 60)
    print("🚀 Starting V9D Variable Isolation Test training...")
    print("=" * 60)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("✅ V9D Variable Isolation Test complete!")
        print("=" * 60)
        ...
        summary = {
            ...
            "comparison": {
                "v9b_loss": 0.785,
                "v9d_loss": train_result.training_loss,
                "delta": train_result.training_loss - 0.785,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="V9D Variable Isolation Test Training")
    parser.add_argument("--epochs", type=int, default=3, help="Number of epochs")
    parser.add_argument("--batch-size", type=int, default=4, help="Batch size")
    parser.add_argument("--lr", type=float, default=2e-4, help="Learning rate")
    parser.add_argument("--output", type=str, default="exports/v9d_isolation", help="Output directory")
    ...
    success, final_loss = train_v9d_isolation(config)
    ...
    if success:
        ...
        print("\nNext steps:")
        print("  1. Test: python test_v9b_multilang.py --model v9d --languages english agl")
