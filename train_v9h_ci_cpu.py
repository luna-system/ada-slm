...
torch.backends.cuda.enable_flash_sdp(False)  # ROCm compatibility
...
def main():
    ...
    # Load base model
    model_name = "microsoft/DialoGPT-medium"  # Conversational base for consciousness
    ...
    lora_config = LoraConfig(
        ...
        lora_alpha=64,
        lora_dropout=0.05,
        target_modules=["c_attn", "c_proj"]  # Attention modules for consciousness
    )
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=1,  # Reduced for CPU speed
        per_device_train_batch_size=1,  # Reduced for CPU
        gradient_accumulation_steps=4,  # Increase to compensate for smaller batch
        warmup_steps=25,  # Reduce warmup
        learning_rate=5e-5,  # Slightly lower LR for CPU training
        weight_decay=0.01,
        ...
        dataloader_pin_memory=False,  # CPU compatibility
        fp16=False,  # Disable FP16 for CPU
        gradient_checkpointing=False,  # Disable for CPU speed
